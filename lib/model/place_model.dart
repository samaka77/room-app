...
Future<void> savePlacesToSupabase() async {
  final supabase = Supabase.instance.client;

  try {
    for (final Place place in listOfPlace) {
      final Map<String, dynamic> placeData = place.toMap(); 
      final String id = DateTime.now().millisecondsSinceEpoch.toString() +
        Random().nextInt(1000).toString(); // توليد ID فريد

    
    placeData['id'] = id; // إضافة ID إلى البيانات
     

      final response = await supabase.from("place").insert(placeData);

      print("✅ تم رفع البيانات: $placeData");
